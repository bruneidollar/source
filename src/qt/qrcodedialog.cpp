...
{
    QString ret = QString("bruneidollar:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BND unit input in lnReqAmount, we generate the URI with BND as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(bruneidollarUnits::format(bruneidollarUnits::BND, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
