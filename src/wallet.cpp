...
// bruneidollar: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-bruneidollar-address
...
// BruneiDollar: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CbruneidollarAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CbruneidollarAddress(address).ToString());
}
...
// bruneidollar: check 'spent' consistency between wallet and txindex
// bruneidollar: fix wallet spent state according to txindex
...
// bruneidollar: disable transaction (only for coinstake)
